...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cellString length] > 0) {
        ...
        if (model.emModel.direction == EMMessageDirectionSend) {
            [cell.avatarView sd_setImageWithURL:[NSURL URLWithString:[[LTSCTool ShareTool].userModel.userHead stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet  URLQueryAllowedCharacterSet]]] placeholderImage:[UIImage imageNamed:@"user"] options:SDWebImageRetryFailed];
        }else {
