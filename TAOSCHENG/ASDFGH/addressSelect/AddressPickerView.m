...
static CGFloat TITLEHEIGHT = 44.0;// 标题栏高度
static CGFloat TITLEBUTTONWIDTH = 120;// 按钮宽度
...
- (void)loadUI {
    ...
    //加载选择器
    [self loadPickerView];
    [self bringSubviewToFront:self.titleBackgroundView];
//    if (AD_iPhoneX) {
...
- (UIPickerView *)addressPickerView{
    if (!_addressPickerView) {
        _addressPickerView = [[UIPickerView alloc]initWithFrame:
                              CGRectMake(0, self.titleBackgroundView.frame.origin.y, SELFSIZE.width, CONTENTHEIGHT - TITLEHEIGHT)];
...
- (void)loadTitle{
    ...
    [self.titleBackgroundView addSubview:line];
    [line mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.leading.trailing.equalTo(self.titleBackgroundView);
...
- (UIView *)pickerView:(UIPickerView *)pickerView
    ...
    if (!pickerLabel){
        ...
        pickerLabel.textColor = CharacterDarkColor;
...
- (void)showOrHide:(BOOL)isShow animation:(BOOL)animation{
    ...
    if (!animation) {
        ...
        self.addressPickerView.frame = CGRectMake(0, self.titleBackgroundView.frame.origin.y, AD_SCREEN.width, CONTENTHEIGHT);
        if (AD_iPhoneX) {
            self.maskingView.frame = CGRectMake(0, self.addressPickerView.frame.origin.y + self.addressPickerView.frame.size.height, AD_SCREEN.width, 34);
        }
        return;
